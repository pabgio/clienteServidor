...
export default function OccurrenceFormEdit({ occurrenceData }) {
  const [id, setId] = useState(occurrenceData.id); // [1
  const [registeredAt, setRegisteredAt] = useState(occurrenceData.registered_at);
  const [local, setLocal] = useState(occurrenceData.local);
  const [km, setKm] = useState(occurrenceData.km);
  const [occurrenceType, setOccurrenceType] = useState(
    String(occurrenceData.occurrence_type)
  );
  const currentDateTime = new Date().toISOString().slice(0, 16);

  const { editarOcorrencia } = useOcorrencias();
  const [isEditing, setIsEditing] = useState(true); // Agora inicia em modo de edição
  ...
  const handleSubmit = async (event) => {
    ...
    editarOcorrencia (id, ocorrenciaData); // 
  };
